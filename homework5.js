...
// <!-- Задание 5.6. Обратитесь к элементу с классом .mt-0 и получите его родителя - элемент с классом .media-body. -->
function example56() {
...
// <!-- Задание 5.7. Добавьте в div c помощью appendChild следующую разметку:
//    <h3>hello world</h3>  . -->
function example57() {
...
// <!-- Задание 5.9. Найдите кнопку с классом .btn-info и сделайте текст в кнопке: "Кнопка". -->
function example59() {
...
function example525() {
    ...
    console.log(div525.classList.contains('hidden') + ` - у элемента есть класс 'hidden'`); //обращение к списку классов с последующим поиском нужного класса
    //true - значит класс hidden есть у данного элемента
};
...
function example528() {
    // TODO! начать тут    
    let div528 = document.querySelectorAll('.exam527 a');//получение коллекции ссылок
    for (let i = 0; i < div528.length; i++) { //перебор коллекции
        if (div528[i].textContent !== 'ссылка') { //поиск совпадений текста в каждом элементе коллекции
            div528[i].innerText = 'ссылка';//добавление текста при отсутствии
        };
    };
};
